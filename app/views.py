...
def ResultFilter(request):
    if request.user.is_authenticated:
        ...
        return render(
            request, "user/results-new.html", {"student": student, "sessions": sessions_from_earliest}
        )
    ...
    if request.method == "POST":
        ...
        if len(new_password) < 8:
            return render(
                ...
                {"error": "Password too short, > than 8 characters!"},
            )
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if p_name != "" and p_duration != "" and p_degree != "":
                try:
                    programmes = Programme.objects.filter(id=p_id)[0]
                ...
                try:
                    ...
                    deptObjects = Department.objects.create(
                        ...
                        college=get_object_or_404(
                            College, id="e3aba966-49f1-40b1-a07a-7a79e32aac5d"
                        ),
                    )
                    deptObjects.save()
...
def UpdateDepartment(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if dept_name != "":
                try:
                    department = Department.objects.filter(id=dept_id)[0]
...
def CourseDept(request):
    if request.user.is_authenticated:
        ...
        return render(
            ...
            {"depts": depts, "department": "instructor"},
        )
...
def adminCourseManagement(request, dept):
    if request.user.is_authenticated:
        ...
        cObjects = Course.objects.all().filter(
            department=get_object_or_404(Department, id=dept)
        )
        course_levels = []
        for x in cObjects:
        ...
        programmes = Programme.objects.all().filter(
            department=get_object_or_404(Department, id=dept)
        )
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if sess != "" and semes != "":
                ...
                register = Registration.objects.all().filter(
                    session=sess,
                    semester=semes,
                    course=get_object_or_404(Course, id=id),
                )
...
def upload_csv(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if (
                get_object_or_404(Session, year=session_year) == current_session_model
                and get_object_or_404(Semester, name=semester_name)
        ...
        if request.method == "POST":
            ...
            if matricNo != "":
                try:
                    ...
                    if student.exists():
                        stu = student.first()
                        registers = Registration.objects.all().filter(
                            student=get_object_or_404(
                                Student, matricNumber=stu.matricNumber
                            )
                        )
                        reg_levels = []
                        for x in registers:
            ...
            if matricNo != "":
                try:
                    ...
                    if student:
                        ...
                        for registration in registrations:
                            res = (
                                Result.objects.filter(registration__id=registration.id)
...
def studentGradeUpdate(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            try:
                latest_result = (
                    Result.objects.filter(registration__id=registrationIdInput)
                    .order_by("-attempt_number")
                    .first()
                )
                ...
                resit_result = Result.objects.create(
                    registration=get_object_or_404(
                        Registration, id=registrationIdInput
                    ),
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            if student:
                ...
                current_session_model = Session.objects.filter(is_current=True).first()
                current_semester_model = Semester.objects.filter(
                    is_current=True
                ).first()
                print(
                    "semester",
                ...
                print(
                    ...
                    type(curr_session),
                    type(current_session_model),
                    curr_session == current_session_model,
                )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # elif user is not None and user.user_type == 'student':
                ...
                return render(
                    request, "authentication/login.html", {"error": error_message}
                )
...
def ParentResultFilter(request):
    ...
    if request.method == "POST":
        ...
        try:
            session = get_object_or_404(Session, year=sess)
            semester = get_object_or_404(Semester, name=semes)
            student = Student.objects.get(matricNumber=matricNumber.strip().lower())
        except Exception as e:
            messages.error(request, f"Input valid details: {e}")
    ...
    return render(
        request, "parents/results-new.html", {"sessions": earliest_session}
    )
...
def ParentResultView(request):
    ...
    return render(request, "parents/resultfilter.html")
